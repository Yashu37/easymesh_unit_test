...
dm_scan_result_t *dm_easy_mesh_t::create_new_scan_result(em_scan_result_id_t *id)
{
	...
	res->map_key = strdup(key);   // 🔥 store key inside object
	...
	hash_map_put(m_scan_result_map, res->map_key, res);

	// 🔥 track allocation
	m_scan_result_list.push_back(res);
	return res;
}

void dm_easy_mesh_t::clear_scan_results()
{
	for (auto *res : m_scan_result_list) {
		if (res) {
			if (res->map_key)
				free(res->map_key);
			delete res;
...
void em_channel_t::fill_scan_result(dm_scan_result_t *scan_res, em_channel_scan_result_t *res, unsigned char *start, unsigned char *end)
{
	unsigned char *tmp;
	em_neighbor_t *nbr;
	...
	size_t max = sizeof(scan_res->m_scan_result.timestamp);

	if (res->timestamp_len > max) {
		printf("Invalid TLV: timestamp too long\n");
		return;
	}

	__asan_poison_memory_region(&scan_res->m_scan_result.timestamp[sizeof(scan_res->m_scan_result.timestamp)],256);
	...
	strncpy(scan_res->m_scan_result.timestamp, (char *)res->timestamp, max - 1);
	scan_res->m_scan_result.timestamp[max - 1] = '\0';	

	__asan_unpoison_memory_region(&scan_res->m_scan_result.timestamp[sizeof(scan_res->m_scan_result.timestamp)],256);
//	tmp = reinterpret_cast<unsigned char *> (res) + sizeof(em_channel_scan_result_t) + res->timestamp_len;
	
	tmp = start + sizeof(em_channel_scan_result_t) + res->timestamp_len;
	printf("DEBUG: struct_size=%zu expected=10\n", sizeof(em_channel_scan_result_t));

	//if (tmp + 1 > end) return;

	__asan_poison_memory_region(&scan_res->m_scan_result.util + 1, 1);
	memcpy(&scan_res->m_scan_result.util, tmp, sizeof(unsigned char));
	__asan_unpoison_memory_region(&scan_res->m_scan_result.util + 1, 1);
	tmp += sizeof(unsigned char);

	//if (tmp + 1 > end) return;

	__asan_poison_memory_region(&scan_res->m_scan_result.noise + 1, 1);	
	memcpy(&scan_res->m_scan_result.noise, tmp, sizeof(unsigned char));
	__asan_unpoison_memory_region(&scan_res->m_scan_result.noise + 1, 1);
	tmp += sizeof(unsigned char);

    //check 🔥 
	if (tmp + sizeof(unsigned short) > end) {
		printf("Invalid TLV: no num_neighbors field\n");
		return ;
	}

	__asan_poison_memory_region((unsigned char *)&scan_res->m_scan_result.num_neighbors + 2, 2);
	memcpy(&scan_res->m_scan_result.num_neighbors, tmp, sizeof(unsigned short));
	scan_res->m_scan_result.num_neighbors = ntohs(scan_res->m_scan_result.num_neighbors);
	__asan_unpoison_memory_region((unsigned char *)&scan_res->m_scan_result.num_neighbors + 2, 2);
	...
	for (i = 0; i < scan_res->m_scan_result.num_neighbors; i++) {
		...
		__asan_poison_memory_region(&nbr->bssid[6], 6);
		memcpy(nbr->bssid, tmp, sizeof(mac_address_t));
		__asan_unpoison_memory_region(&nbr->bssid[6], 6);
		tmp += sizeof(mac_address_t);

		__asan_poison_memory_region((unsigned char *)&ssid_len + 1, 1);
		memcpy(&ssid_len, tmp, sizeof(unsigned char));
		__asan_unpoison_memory_region((unsigned char *)&ssid_len + 1, 1);
		tmp += sizeof(unsigned char);

		printf("Reached SSID copy\n");

		printf("DEBUG: ssid_len=%u, remaining=%ld\n", ssid_len, (long)(end - tmp));

		// 🔥 ADD THIS CHECK
		if ((tmp + ssid_len > end) || (ssid_len > sizeof(nbr->ssid))) {
				printf("Invalid TLV: SSID error\n");
				return;
		}

		//  Poison bytes after valid SSID (32 bytes buffer)
		__asan_poison_memory_region(&nbr->ssid[31] + 1, 256);
		//strncpy(nbr->ssid, reinterpret_cast<char *> (tmp), static_cast<size_t>(ssid_len + 1));
		//nbr->ssid[ssid_len] = '\0';
		// Unpoison after write
		size_t copy_len = ssid_len;

		if (copy_len >= sizeof(nbr->ssid)) {
			copy_len = sizeof(nbr->ssid) - 1;   // keep space for '\0'
		}

		strncpy(nbr->ssid, (char *)tmp, copy_len);
		nbr->ssid[copy_len] = '\0';
		
		__asan_unpoison_memory_region(&nbr->ssid[31] + 1, 256);
		tmp += ssid_len;

		__asan_poison_memory_region((unsigned char *)&nbr->signal_strength + 1, 1);
		memcpy(&nbr->signal_strength, tmp, sizeof(unsigned char));
		__asan_unpoison_memory_region((unsigned char *)&nbr->signal_strength + 1, 1);
		tmp += sizeof(unsigned char);

		__asan_poison_memory_region((unsigned char *)&bw_len + 1, 256);
		memcpy(&bw_len, tmp, sizeof(unsigned char));
		__asan_unpoison_memory_region((unsigned char *)&bw_len + 1, 256);
		tmp += sizeof(unsigned char);

		// 🔥 ADD THIS
		if ((tmp + bw_len > end) || (bw_len > sizeof(bandwidth))) {
				printf("Invalid TLV: bandwidth error\n");
				return;
		}

		__asan_poison_memory_region(&bandwidth[31] + 1, 256);
		memcpy(bandwidth, tmp, bw_len);
		__asan_unpoison_memory_region(&bandwidth[31] + 1, 256);
		tmp += bw_len;
		...
		__asan_poison_memory_region((unsigned char *)&nbr->bss_color + 1, 1);
		memcpy(&nbr->bss_color, tmp, sizeof(unsigned char));
		__asan_unpoison_memory_region((unsigned char *)&nbr->bss_color + 1, 1);
		tmp += sizeof(unsigned char);

		__asan_poison_memory_region((unsigned char *)&nbr->channel_util + 1, 1);
		memcpy(&nbr->channel_util, tmp, sizeof(unsigned char));
		__asan_unpoison_memory_region((unsigned char *)&nbr->channel_util + 1, 1);
		tmp += sizeof(unsigned char);

		//cheks 🔥 
		if (tmp + sizeof(unsigned short) > end) {
			printf("Invalid TLV: no sta_count\n");
			return;
		}

		__asan_poison_memory_region((unsigned char *)&nbr->sta_count + 2, 2);
		memcpy(&nbr->sta_count, tmp, sizeof(unsigned short));
		nbr->sta_count = ntohs(nbr->sta_count);
		__asan_unpoison_memory_region((unsigned char *)&nbr->sta_count + 2, 2);
		tmp += sizeof(unsigned short);
	...
	//check 🔥
	if (tmp + sizeof(unsigned int) > end) {
		printf("Invalid TLV: no aggr_scan_duration\n");
		return;
	}

	__asan_poison_memory_region((unsigned char *)&scan_res->m_scan_result.aggr_scan_duration + 4, 4);
	memcpy(&scan_res->m_scan_result.aggr_scan_duration, tmp, sizeof(unsigned int));
	__asan_unpoison_memory_region((unsigned char *)&scan_res->m_scan_result.aggr_scan_duration + 4, 4);
	tmp += sizeof(unsigned int);

	__asan_poison_memory_region((unsigned char *)&scan_res->m_scan_result.scan_type + 1, 1);
	memcpy(&scan_res->m_scan_result.scan_type, tmp, sizeof(unsigned char));
	__asan_unpoison_memory_region((unsigned char *)&scan_res->m_scan_result.scan_type + 1, 1);
	tmp += sizeof(unsigned char);
...
int em_channel_t::handle_channel_scan_rprt(unsigned char *buff, unsigned int len)
{
    ...
    printf("Function: %s Line: %d Start TLV parsing tlv_len=%d\n", __func__, __LINE__, tlv_len);

    // 🔥 FIXED WHILE (minimal change)
    while (tlv_len >= (int)sizeof(em_tlv_t) && tlv->type != em_tlv_type_eom)
    {
        uint16_t value_len = ntohs(tlv->len);

        if (tlv->type == em_tlv_type_channel_scan_rslt) {
            //check 🔥 
            if (value_len < sizeof(em_channel_scan_result_t)) {
                printf("Invalid TLV: too small for channel_scan_result\n");
                return -1;
            }
            ...
            unsigned char *start = tlv->value;
            unsigned char *end   = start + value_len;

            fill_scan_result(scan_res, res, start, end);

        }

        if (tlv->type == em_tlv_type_timestamp) {
            printf("Function: %s Line: %d Timestamp TLV found\n", __func__, __LINE__);
        }

        // 🔥 Minimal safety for TLV length
        if (value_len > (uint16_t)(tlv_len - sizeof(em_tlv_t))) {
            printf("Invalid TLV length\n");
            break;
        }

        int step = sizeof(em_tlv_t) + value_len;

        tlv_len -= step;
        tlv = reinterpret_cast<em_tlv_t *> (reinterpret_cast<unsigned char *>(tlv) + step);
    }
